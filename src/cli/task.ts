...
import { storage, TaskConfig } from "../lib/storage";
...
function cmdList() {
  ...
  for (const task of tasks) {
    ...
    console.log(
      `  ${colors.blue}Status:${colors.NC} ${statusColor}${task.status}${colors.NC}`
    );
    console.log("");
  }
...
function cmdStatus(taskId: string) {
  ...
  console.log(
    `  ${colors.blue}Run Count:${colors.NC} ${task.state.runCount}`
  );
  console.log(
    `  ${colors.blue}Last Run:${colors.NC} ${task.state.lastRun || "None"}`
  );
  console.log(
    `  ${colors.blue}Last Result:${colors.NC} ${task.state.lastResult || "N/A"}`
  );
...
function cmdNotes(taskId: string, lines?: number) {
  ...
  const size = storage.getNotesSize(taskId);
...
function cmdCreate(
  ...
  if (!templates.includes(template)) {
    log.error(
      `Template not found / 找不到模板: ${template}`
    );
    console.log(
      `  Available templates / 可用模板: ${templates.join(", ")}`
    );
    process.exit(1);
  }
...
function doDelete(taskId: string, task: TaskConfig) {
  if (task.cronJobId) {
    try {
...
function cmdCompact(taskId: string) {
  ...
  log.success(
    `Notes compacted / 筆記已壓縮: ${Math.round(result.oldSize / 1024)} KB → ${Math.round(result.newSize / 1024)} KB`
  );
}
  ...
  switch (command) {
    case "create": {
      ...
      if (!template || !name || !schedule) {
        ...
        console.log(
          "Usage: openclaw task create --template <tmpl> --name <name> --schedule <cron> [--agent <agent>]"
        );
        process.exit(1);
      }
